...
class nbow(object):
    def __init__(self, sentence_len, vocab_size, embedding_size, num_label):
        ...
        with tf.name_scope("loss"):
            self.losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.logits, labels=self.label)
            self.loss = tf.reduce_mean(self.losses)

        # with tf.name_scope("prediction"):
        #     self.equal = tf.cast(tf.equal(tf.argmax(self.logits, axis=1), tf.argmax(self.label, axis=1)), dtype=tf.float32)
